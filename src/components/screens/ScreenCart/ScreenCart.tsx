...
import RedireccionOverlay from "../../ui/RedireccionOverlay/RedireccionOverlay";
...
export const ScreenCart = () => {
	const { items, eliminar, total, cambiarCantidad, vaciar } = useCartStore();
	const [stockMap, setStockMap] = useState<Record<number, number>>({});
	const [direccionSeleccionadaId, setDireccionSeleccionadaId] = useState<number | null>(null);
	const [mostrandoRedireccion, setMostrandoRedireccion] = useState(false);
	...
	const handlePagarUnificado = async () => {
		...
		try {
			...
			items.forEach((item) => {
				console.log("productoId:", item.detalleId, "cantidad:", item.cantidad);
			});
