...
const ScreenProductPage = () => {
	...
	const [productosRelacionados, setProductoRelacionados] = useState<IDetalleDTO | any>();
	...
	if (!producto) return <h2 style={{ textAlign: "center" }}>Cargando producto...</h2>;
	...
	const calcularDescuento = (precioVenta: number, porcentajeDescuento: number) => {
	...
	const getDetallesPorProducto = async () => {
		try {
			const detalles = await detalleService.getDetallesPorProducto(producto.producto.id);
